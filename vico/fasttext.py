...
class FastVector:
    ...
    def apply_transform(self, transform):
        ...
        or a numpy ndarray.
        """
        transmat = np.loadtxt(transform) if isinstance(transform,
                                                       str) else transform
    ...
    def translate_nearest_neighbour(self, source_vector):
        ...
        similarity_vector = np.matmul(FastVector.normalised(self.embed),
                                      source_vector)
    ...
    def get_samples(self, nsamples):
        ...
        sample_ids = np.random.choice(self.embed.shape[0], nsamples,
                                      replace=False)
